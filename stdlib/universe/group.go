...
func NewGroupTransformation(spec *GroupProcedureSpec, id execute.DatasetID, mem *memory.Allocator) (execute.Transformation, execute.Dataset) {
	t := &groupTransformation{
		cache: tableutil.BuilderCache{
			New: func(key flux.GroupKey) tableutil.Constructor {
				return tableutil.NewBufferedTableBuilder(key, mem)
			},
		},
		mem:  mem,
		mode: spec.GroupMode,
	...
	cache := &tableutil.BuilderDataCache{BuilderCache: &t.cache}
	t.d = execute.NewDataset(id, execute.DiscardingMode, cache)
	sort.Strings(t.keys)
	return t, t.d
}
...
func (t *groupTransformation) Process(id execute.DatasetID, tbl flux.Table) error {
	...
	if key, ok, err := t.getTableKey(tbl); err != nil {
		...
		ab, _ := tableutil.GetBufferedTableBuilder(key, &t.cache)
...
func (t *groupTransformation) appendTable(ab *tableutil.BufferedTableBuilder, tbl flux.Table) error {
...
func (t *groupTransformation) groupByRow(tbl flux.Table) error {
	...
	cache := tableutil.BuilderCache{
		New: func(key flux.GroupKey) tableutil.Constructor {
			return tableutil.NewArrowBuilder(key, t.mem)
		},
	}
	if err := tbl.Do(func(cr flux.ColReader) error {
		for i, l := 0, cr.Len(); i < l; i++ {
			key := execute.GroupKeyForRowOn(i, cr, on)
			ab, created := tableutil.GetArrowBuilder(key, &cache)
			if created {
	...
	return cache.ForEachBuilder(func(key flux.GroupKey, builder tableutil.Constructor) error {
